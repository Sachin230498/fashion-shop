...
export const Search = () => {
  ...
  const handleFetch = (val) => {
    getData(`https://aakash.onrender.com/products?category=${val}`)
      .then((res) => {
        setIsLoading(true);
        setData(res.data);
        setpData(res.data);
  ...
  useEffect(() => {
    getData("https://aakash.onrender.com/products")
      .then((res) => {
        setIsLoading(true);
        setData(res.data);
        setpData(res.data);
      ...
      >
        ...
        <DrawerContent>
          ...
          <DrawerFooter>
            ...
            <Button colorScheme="blue" onClick={onClosef}>
              Save
            </Button>
          </DrawerFooter>
        </DrawerContent>
                  ...
                  return (
                    ...
                    >
                      <Skeleton isLoaded={isloading}>
                        <RouterLink to={`/productsDetails/${el.id}`}>
                        ...
                        >
                          <Box>{el.title}</Box> <Box>&#8377; {el.price}</Box>
                        </Box>
                      </Skeleton>
                    </Box>
                  );
                ...
                data.map((el) => {
                  return (
                    ...
                    >
                      <Skeleton isLoaded={isloading}>
                        <RouterLink to={`/productsDetails/${el.id}`}>
                          <Image w="100%" src={el.image} alt={el.id} />
                        </RouterLink>
                        {isShown && (
                          <Button
                            variant="ghost"
